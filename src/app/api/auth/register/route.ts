...
export const POST = async (req: NextRequest) => {
    try {
        const { username, name, email, password, passwordConfirm, userRole } = await req.json();

        if (!email || !email.includes("@") || !password || password.trim().length < 7 || !username || !name || !userRole) {
