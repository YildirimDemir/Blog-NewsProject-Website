...
export async function GET(req: NextRequest, { params }: { params: { commentId: string } }) {
    ...
    try {
        ...
        if (error instanceof Error) {
            ...
            return NextResponse.json({ message: 'Failed to fetch comment', details: error.message }, { status: 500 });
        } else {
            console.error('An unknown error occurred');
            return NextResponse.json({ message: 'Failed to fetch comment', details: 'An unknown error occurred' }, { status: 500 });
        }
    }
}
